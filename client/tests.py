...
class TestSAKo(unittest.TestCase):
    ...
    def test_SAKo(self):
        os.system('python '+ self.path_to_script+'/SAKo.py -d '+ self.path_to_script+'/example -l test -p test1 -a sako_dev -t ping > test.txt')
        p_f = open('test.txt', 'r')
        text = p_f.read()[-4:-2]
        print text
    ...
    def test_submit(self):
        result = SAKo.submit('sako_dev', self.path_to_script+'/example', 'test', 'test1',
                             'ping')
